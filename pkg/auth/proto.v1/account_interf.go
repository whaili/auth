...
const (
	...
	USER_TYPE_UNREGISTERED = 0x10000
	USER_TYPE_BUFFERED     = 0x20000
	USER_TYPE_OVERSEAS     = 0x40000 // 海外用户
	USER_TYPE_OVERSEAS_STD = 0x80000 // 海外标准用户
...
// IsOverseasUser 是海外用户
func (t UserType) IsOverseasUser() bool {
	return t&USER_TYPE_OVERSEAS != 0
}

// IsOverseasStd 是海外标准用户，海外标准用户一定是海外用户
...
type Interface interface {
	GetAccessInfo(ctx context.Context, accessKey string) (ret AccessInfo, err error)
	GetUtype(ctx context.Context, uid uint32) (utype uint32, err error)
}
