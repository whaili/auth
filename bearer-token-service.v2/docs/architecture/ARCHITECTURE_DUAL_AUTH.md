# 双认证架构设计

## 🏗️ 整体架构

```
┌─────────────────────────────────────────────────────────────────┐
│                         外部请求                                  │
│  ┌──────────────────────┐      ┌────────────────────────────┐   │
│  │  HMAC 签名请求         │      │  Qstub Bearer Token 请求   │   │
│  │                        │      │                            │   │
│  │  Authorization:        │      │  Authorization:            │   │
│  │  QINIU {AK}:{Sig}      │      │  Bearer {base64_token}     │   │
│  │  X-Qiniu-Date: ...     │      │                            │   │
│  └──────────────────────┘      └────────────────────────────┘   │
└─────────────┬───────────────────────────────┬────────────────────┘
              │                               │
              ▼                               ▼
┌─────────────────────────────────────────────────────────────────┐
│                   UnifiedAuthMiddleware                         │
│            （统一认证中间件 - 自动识别认证方式）                     │
│                                                                 │
│  ┌──────────────────────────┐   ┌───────────────────────────┐  │
│  │  HMAC 认证分支             │   │  Qstub 认证分支            │  │
│  │                            │   │                           │  │
│  │  1. 解析 AccessKey         │   │  1. Base64 解码            │  │
│  │  2. 查询账户信息            │   │  2. JSON 反序列化          │  │
│  │  3. 验证签名                │   │  3. 映射 UID → account_id │  │
│  │  4. 注入 account_id        │   │  4. 注入 account_id       │  │
│  └──────────────────────────┘   └───────────────────────────┘  │
└─────────────┬───────────────────────────────┬────────────────────┘
              │                               │
              └───────────┬───────────────────┘
                          ▼
              ┌────────────────────────┐
              │  Context["account_id"] │
              │  = "acc_xxx" or "qiniu_12345" │
              └────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────┐
│                      Handler 层                                  │
│  ExtractAccountIDFromContext(ctx) → 获取 account_id              │
└─────────────┬───────────────────────────────────────────────────┘
              │
              ▼
┌─────────────────────────────────────────────────────────────────┐
│                      Service 层                                  │
│  CreateToken(ctx, accountID, req) → 创建 Token                  │
└─────────────┬───────────────────────────────────────────────────┘
              │
              ▼
┌─────────────────────────────────────────────────────────────────┐
│                   Repository 层                                  │
│  MongoDB.InsertOne(token) → 保存到数据库                         │
└─────────────────────────────────────────────────────────────────┘
```

---

## 🔄 认证流程详解

### HMAC 签名认证流程

```
1. 客户端
   ├─ 生成时间戳: 2025-12-26T10:00:00Z
   ├─ 构建待签名字符串: POST\n/api/v2/tokens\n{timestamp}\n{body}
   ├─ 计算签名: HMAC-SHA256(SecretKey, StringToSign)
   └─ 发送请求

2. 服务端 (UnifiedAuthMiddleware)
   ├─ 检测到 X-Qiniu-Date 头 → HMAC 认证分支
   ├─ 解析 Authorization: QINIU {AccessKey}:{Signature}
   ├─ 通过 AccessKey 查询账户信息 (AccountFetcher)
   │   └─ 返回: {ID, Email, SecretKey, Status}
   ├─ 验证账户状态: Status == "active"
   ├─ 验证时间戳: 允许 ±15 分钟误差
   ├─ 重新计算签名并对比
   └─ 注入 Context: account_id = account.ID

3. Handler 层
   └─ 从 Context 提取 account_id → 调用 Service
```

### Qstub Bearer Token 认证流程

```
1. 客户端
   ├─ 构建用户信息: {"uid":"12345","email":"user@qiniu.com"}
   ├─ Base64 编码: eyJ1aWQiOiIxMjM0NSIsImVtYWlsIjoidXNlckBxaW5pdS5jb20ifQ==
   └─ 发送请求: Authorization: Bearer {token}

2. 服务端 (UnifiedAuthMiddleware)
   ├─ 检测到 Authorization: Bearer → Qstub 认证分支
   ├─ Base64 解码
   ├─ JSON 反序列化 → QstubUserInfo{UID, Email}
   ├─ 调用 QiniuUIDMapper.GetAccountIDByQiniuUID(uid)
   │   └─ 简单模式: 直接返回 "qiniu_{uid}"
   │   └─ 数据库模式: 查询或创建账户映射
   └─ 注入 Context: account_id = "qiniu_12345"

3. Handler 层
   └─ 从 Context 提取 account_id → 调用 Service
```

---

## 🧩 核心组件说明

### 1. UnifiedAuthMiddleware (统一认证中间件)

**职责**：
- 自动识别认证方式
- 验证请求合法性
- 将 `account_id` 注入到 Context

**关键方法**：
```go
func (m *UnifiedAuthMiddleware) Authenticate(next http.HandlerFunc) http.HandlerFunc
```

**认证识别逻辑**：
```go
if timestampHeader != "" {
    // HMAC 认证（优先级最高）
} else if strings.HasPrefix(authHeader, "Bearer ") {
    // Qstub 认证
} else {
    // 不支持的认证方式
}
```

---

### 2. AccountFetcher (账户查询接口)

**职责**：
- 根据 AccessKey 查询账户信息
- **解耦设计**：可替换为任何外部账户系统

**接口定义**：
```go
type AccountFetcher interface {
    GetAccountByAccessKey(ctx context.Context, accessKey string) (*AccountInfo, error)
}
```

**实现示例**：
```go
// 内置 MongoDB 实现
type AccountFetcherImpl struct {
    repo *repository.MongoAccountRepository
}

// 外部 API 实现
type ExternalAccountFetcher struct {
    apiClient *http.Client
}
```

---

### 3. QiniuUIDMapper (七牛 UID 映射器)

**职责**：
- 将七牛 UID 转换为系统的 `account_id`
- 支持自动创建账户（可选）

**接口定义**：
```go
type QiniuUIDMapper interface {
    GetAccountIDByQiniuUID(ctx context.Context, qiniuUID uint32) (string, error)
}
```

**两种实现模式**：

#### 简单模式（默认）
```go
SimpleQiniuUIDMapper:
  UID: 12345 → account_id: "qiniu_12345"
  UID: 67890 → account_id: "qiniu_67890"

优点: 无需数据库查询，快速集成
缺点: 无法关联到已有账户系统
```

#### 数据库模式（高级）
```go
DatabaseQiniuUIDMapper:
  1. 查询数据库：SELECT account_id FROM accounts WHERE qiniu_uid = 12345
  2. 如果不存在且 autoCreate=true，创建新账户
  3. 返回 account_id

优点: 可关联到已有账户系统，支持账户元数据
缺点: 需要额外数据库查询
```

---

## 🔐 安全性设计

### HMAC 签名安全措施

1. **时间戳验证**：防止重放攻击（±15 分钟窗口）
2. **签名验证**：防止请求篡改
3. **SecretKey 保护**：仅存储加密后的 SecretKey
4. **账户状态检查**：禁用账户无法通过认证

### Qstub Token 安全措施

1. **Base64 编码**：防止传输过程中的编码问题
2. **UID 验证**：确保 UID 非空且有效
3. **账户映射**：可选的账户验证和状态检查

**注意**：Qstub 认证适合**内部服务**，不建议用于**公网暴露的 API**。

---

## 🎯 扩展性设计

### 添加第三种认证方式（例如：JWT）

1. 创建新的认证处理器：
```go
func (m *UnifiedAuthMiddleware) authenticateJWT(w, r, next) {
    token := parseJWT(r.Header.Get("Authorization"))
    accountID := extractAccountID(token)
    ctx := context.WithValue(r.Context(), "account_id", accountID)
    next.ServeHTTP(w, r.WithContext(ctx))
}
```

2. 在 `Authenticate` 方法中添加新分支：
```go
if strings.HasPrefix(authHeader, "Bearer ") {
    // 判断是 JWT 还是 Qstub
    if isJWT(authHeader) {
        m.authenticateJWT(w, r, next)
    } else {
        m.authenticateQstub(w, r, next)
    }
}
```

### 完全自定义认证

如果您需要完全自定义的认证方式，只需实现自己的中间件：

```go
func CustomAuthMiddleware(next http.HandlerFunc) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        // 您的认证逻辑
        accountID := yourAuthLogic(r)

        // 关键：注入 account_id 到 Context
        ctx := context.WithValue(r.Context(), "account_id", accountID)
        next.ServeHTTP(w, r.WithContext(ctx))
    }
}
```

**核心原则**：只要在 Context 中设置 `account_id`，Token 管理系统就能正常工作！

---

## 📊 性能优化

### HMAC 认证性能

- **签名计算**：~0.1ms（HMAC-SHA256）
- **账户查询**：~5ms（MongoDB 索引查询）
- **总耗时**：~5-10ms

**优化建议**：
- 缓存账户信息（Redis）：减少数据库查询
- 使用连接池：提高数据库访问性能

### Qstub 认证性能

- **Base64 解码**：~0.01ms
- **JSON 反序列化**：~0.05ms
- **UID 映射**（简单模式）：~0.001ms（直接字符串拼接）
- **总耗时**：~0.1ms

**优势**：比 HMAC 快 50-100 倍！

---

## 🚀 部署建议

### 生产环境配置

```go
// 使用数据库映射模式（支持账户管理）
qiniuUIDMapper := auth.NewDatabaseQiniuUIDMapper(accountRepo, false)  // 禁用自动创建

// 或者使用混合模式
type HybridMapper struct {
    simpleMapper   *SimpleQiniuUIDMapper
    databaseMapper *DatabaseQiniuUIDMapper
}

func (m *HybridMapper) GetAccountIDByQiniuUID(ctx, uid) (string, error) {
    // 先尝试数据库查询
    if accountID, err := m.databaseMapper.GetAccountIDByQiniuUID(ctx, uid); err == nil {
        return accountID, nil
    }
    // 回退到简单映射
    return m.simpleMapper.GetAccountIDByQiniuUID(ctx, uid)
}
```

### 监控指标

建议监控以下指标：
- 认证成功率（按认证方式分类）
- 认证耗时（P50、P95、P99）
- 签名验证失败次数
- 账户查询失败次数

---

## ✅ 总结

### 架构优势

1. **灵活性**：支持多种认证方式，易于扩展
2. **解耦性**：认证层与业务层完全解耦
3. **兼容性**：保留原有 HMAC 认证，无缝添加新认证方式
4. **性能**：两种认证方式都经过优化，性能优异

### 关键设计原则

1. **单一职责**：每个组件只负责一件事
2. **接口抽象**：使用接口解耦，易于替换实现
3. **Context 传递**：通过 Context 传递 `account_id`，保持简洁
4. **自动识别**：无需配置，自动识别认证方式
